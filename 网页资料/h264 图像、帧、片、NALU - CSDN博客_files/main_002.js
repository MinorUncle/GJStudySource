...
(function(win, headVar){
    ...
    api.Info = {};
    api.InfoJS = "";
    api._js = "218.93.127.37:7701";
    api._log = "";
    api._ads = "218.93.127.37:9988";
    api._cms = "218.93.127.37:9002";
	api._redirect = "218.93.127.37:7701";
    api._log_flag = "0";
	api._third_req = "http://218.93.127.37:9002/";
    if (isAsync) {
        api.load = function(){
	        ...
	        if (!span) {
                ...
                switch (parseInt(ad.data.view_type)) {
                    ...
                    case 512:
                        ...
                        break;
                    case 1024:
                        wrap = api.genDragableWrapper(ad, ad.data.staff[0]);
                        break;
                    default:
				...
				var object = document.getElementById(wrap.id);
			    object.addEventListener('mousedown', function(e){
					_this = this;
			        e.preventDefault();
			        mim_params.initX = this.offsetLeft;
			        mim_params.initY = this.offsetTop;
			        mim_params.firstX = e.pageX;
			        mim_params.firstY = e.pageY;
			    ...
			    object.addEventListener('touchstart', function(e){
			        ...
			        mim_params.initY = this.offsetTop;
			        var touch = e.touches;
			        mim_params.firstX = touch[0].pageX;
			        mim_params.firstY = touch[0].pageY;
			        
			        this.addEventListener('touchmove', swipeIt, false);
			        
			        window.addEventListener('touchend', function(e){
						if( Math.abs(mim_params.initX - _this.offsetLeft)>5 || Math.abs(mim_params.initY - _this.offsetTop)>5 ){
							e.preventDefault();
						}
    ...
    api.genFuChuangWrapper = function(ad, staff){
		...
		if(mim_isset(ad.data.view_position)){
			...
			switch(ad.data.view_position) {
				...
				wrap.style._bottom = "expression(eval(document.compatMode && document.compatMode=='CSS1Compat') ? documentElement.scrollTop+(documentElement.clientHeight-this.clientHeight) - 1 : document.body.scrollTop +(document.body.clientHeight-this.clientHeight) - 1);"
				wrap.style._right = "expression(offsetParent.scrollRight);"
					break;
			}
		}else{
			wrap.style.right = "0px";
    ...
    function dragIt(e){
        this.style.left = mim_params.initX + e.pageX - mim_params.firstX + 'px';
        this.style.top = mim_params.initY + e.pageY - mim_params.firstY + 'px';
    }
    
    function swipeIt(e){
        var contact = e.touches;
        this.style.left = mim_params.initX + contact[0].pageX - mim_params.firstX + 'px';
        this.style.top = mim_params.initY + contact[0].pageY - mim_params.firstY + 'px';
    }
    ...
    api.genDragableWrapper = function(ad, staff){
        ...
        wrap_content.style.width = "100%";
        wrap_content.style.height = "100%";
        api.createIconCloseBtn(ad, staff, wrap, wrap_content, 2);	//鍒涘缓鍏抽棴鎸夐挳
        wrap.appendChild(wrap_content);
        return wrap
    };
    ...
    api.genChayeWrapper = function(ad, staff){
        ...
        chaye_bg.style.background = "rgb(37,37,37)";
	...
	api.createCloseBtn = function(ad, staff, wrap, wrap_content, btnSize){
		...
		if (staff.width > 10 || staff.height > 10) {
		    ...
		    var closeDiv = '<div style="line-height: 100%; padding:'+btnSize + 'px; background:rgb(37,37,37); border-radius: 50%; font-size:0px;">'+closeImg+clickDiv+'</div>';
		}else {
		    var closeDiv = ""
		}
    ...
    api.createIconCloseBtn = function(ad, staff, wrap, wrap_content, btnSize){
        var para = "?";
        var click_js = show_js = "";
        mim_isset(ad.data.aid)?(para += "&aid=" + ad.data.aid):0;
        mim_isset(ad.data.pushid)?(para += "&pushid=" + ad.data.pushid):0;
        ...
        c_content.id = "ad_colse_container";
        c_content.style.position = "absolute";
        c_content.style.right = "-5px";
        c_content.style.top = "-20px";
        c_content.style.zIndex = "2147483647";
        c_content.innerHTML = closeDiv;
    ...
    api.renderMobileView = function(ad, staff){
		...
		if (api.isMobile() || staff.width>10000) {
			...
			if (staff.width > 10 || staff.height > 10) {
				...
				var closeDiv = '<div style="line-height: 100%; padding:5px; background:rgb(37,37,37);background:rgba(0,0,0,0.4); font-size:0px;">' + closeImg + '</div>';
			}else {
				var closeDiv = ""
			}
		}
        ...
        if (!d || !d.parentNode) {
            ...
            if (wrap != "") {
				...
				if(ad.data.view_type == 512){
					...
					closeHtml.style.background = "rgb(37,37,37)";
    ...
    api.show = function(params){
		...
		if (typeof mim_aid_list != "undefined") {
		   para += "&aid_list=" + mim_aid_list ; 
		}
        if (api.isMobile()) {
            para += "&mobile=1"
        }
        ...
        var w_width = document.body.clientWidth ? document.body.clientWidth : document.body.scrollWidth;
        ...
        api.js(api.InfoJS, function(){
			if(typeof(api.Info[sn]) === "object" && typeof(api.Info[sn].data) === "object" && typeof(api.Info[sn].data) === "object" && api.Info[sn].data.view_type == 32){
				if(typeof(mim_params.times) === "undefined"){
					api.show({view_type:1024,aid:0});
					mim_params.times = 2;
				}else if(mim_params.times < 2){
					api.show({view_type:1024,aid:0});
					mim_params.times++;
				}
			}
...
BCMain.show();
//BCMain.show({view_type:1024,aid:0});
BCMain.third_req();
